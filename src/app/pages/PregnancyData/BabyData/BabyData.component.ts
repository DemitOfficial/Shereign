...
@Component({
  selector: 'app-BabyData',
  templateUrl: './BabyData.component.html',
  styleUrls: ['./BabyData.component.scss']
})
export class BabyDataComponent implements OnInit {
...
this.loggedinuser=JSON.parse(localStorage.getItem('AdminData')) as Admin;
  this.spinner.show();
   ...
   this.spinner.hide()
    ...
    this.editbabyweekdata = this.formBuilder.group({
  ...
  Delete(id:any){
    debugger
    this.spinner.show()
    this.firestore.doc('WeekBabyData/'+id).delete();
    this.spinner.hide();
  ...
  getdata(){
    return this.firestore.collection('WeekBabyData').snapshotChanges();
  }
  adminmodal(content){
  ...
  editmodal(content,id:string){
    ...
    this.editbabyweekdata.get('detail').patchValue(data[0].detail);
    this.editbabyweekdata.get('heading').patchValue(data[0].heading);
    this.editbabyweekdata.get('image').patchValue(data[0].image);
    this.modalService.open(content);

  }
  updatedata(){
    this.spinner.show();
    this.firestore.doc('WeekBabyData/'+this.dataid).update(this.editbabyweekdata.value)
    this.editbabyweekdata.reset();
 ...
 async updateimage(){
   this.spinner.show()
    ...
    this.editbabyweekdata.get('detail').patchValue(data[0].detail);
    this.editbabyweekdata.get('heading').patchValue(data[0].heading);
    this.editbabyweekdata.get('image').patchValue(this.url);
    this.firestore.doc('WeekBabyData/'+this.dataid).update(this.editbabyweekdata.value)
    this.editbabyweekdata.reset();
    this.modalService.dismissAll();
    this.spinner.hide();
